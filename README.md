...
- **Dynamic databases** — SQLite tables created on the fly, with secure column types (PASSWORD via bcrypt, ENCRYPTED via AES-256)
- **REST APIs** — Auto-generated CRUD endpoints with filtering, sorting, pagination, and role-based access
- **User auth + OAuth** — JWT-based auth with bcrypt, configurable roles, plus Google/GitHub/Discord/any OAuth 2.0 provider
- **WebSockets** — Bidirectional real-time communication with room-based broadcast relay
...
- Full-text search (FTS5) on any text column
...
| **BUILD** | Single LLM session: creates all tables, endpoints, CSS, layout, JS, and pages with 17 tools |
| **COMPLETE** | Switches to monitoring mode |
| **MONITORING** | Adaptive health checks (5-15 min), investigates and self-heals issues |

Updates use **UPDATE_PLAN** — produces a patch that modifies only affected parts of the plan, then re-enters BUILD to apply changes.
...
| Any OpenAI-compatible | openai | Ollama (local/free), Z.ai, OpenRouter, Groq, etc. |

Providers are configured through the setup wizard or `firstrun.json`.
...
- **GitHub** : [github.com/markdr-hue/IATAN](https://github.com/markdr-hue/IATAN)
